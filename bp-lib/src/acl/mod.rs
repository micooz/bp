...
fn test_load_from_file() {
    ...
    assert!(acl.load_from_file("test/fixtures/acl.txt".into()).is_ok());
}

#[test]
fn test_save_to_file() {
    let acl = AccessControlList::default();
    acl.insert(&String::from("example.com"), DomainRule::NotExtractMatch);
...
fn test_is_host_hit() {
    let acl = AccessControlList::default();
    acl.load_from_file("test/fixtures/acl.txt".into()).unwrap();

    assert!(acl.is_host_hit(&String::from("example.com")));
}
...
fn test_insert() {
    ...
    assert!(acl.is_host_hit(&String::from("example.com")) == false);
}

#[test]
fn test_watch() {
    let acl = AccessControlList::default();
    let path = "test/fixtures/acl.txt".to_string();
